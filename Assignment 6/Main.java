...
public class Main implements ActionListener {
        ...
        private void createAndShowGUI() {
                ...
                map.addGameListener(top);

                state = new StatePanel(300, 15, "Game State");
                state.setPreferredSize(new Dimension(400, 150));

                map.addGameListener(state);

                JPanel topHolder = new JPanel();
                topHolder.setLayout(new BoxLayout(topHolder, BoxLayout.X_AXIS));

                topHolder.add(state);
                topHolder.add(top);

                // PAGE_START puts the panel at the top
                mainPanel.add(topHolder, BorderLayout.PAGE_START);
                ...
                bottom.addZoomButton("Zoom In", map, true);
                bottom.addZoomButton("Zoom Out", map, false);
        ...
        public void actionPerformed(ActionEvent e) {
                ...
                angle.updateAngle();
                map.tickAll(list);
                state.updateGame(map.getNumPlanes(), map.getTotalPlanes() - map.getNumPlanes(), ++time * TICK / 1000.0);
                map.repaint();
